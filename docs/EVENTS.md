...
> Each crate's issue-tracked event work links back here. See the tracked
> "document event schema" issues for `analytics`, `access-control`, `escrow`,
> `governance`, `governance-voting`, `multi-sig-wallet`,
> `time-locked-transactions`, `token-swap`, and `upgrade` for their sections
> as they land.
